    ...
    def update_core(self):
        ...
        if np.array_equal(batch[0][0][0], batch[0][1][0]):
            if isinstance(batch[0][0][0][0], np.int64):
                utt_type = 'tts2asr'
            elif isinstance(batch[0][0][0][0][0], np.float32):
                utt_type = 'asr2tts'
        elif not np.array_equal(batch[0][0][0], batch[0][1][0]):
            utt_type = 'asr'
        else:
            raise NotImplementedError

        if utt_type == 'asr':  # paired data training
        ...
        elif utt_type == 'tts2asr':  # TTS with ASR loss
        ...
        for n, model in enumerate(models):
            ...
            if utt_type == 'asr' and n == 0:  # asr with CE loss
