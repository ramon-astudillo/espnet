...
#        steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 80 data/${x} exp/make_fbank/${x} ${fbankdir}
#    done
...
#        /export/b{14,15,16,17}/${USER}/espnet-data/egs/librispeech/asr-unpaired/dump/${train_set}/delta${do_delta}/storage \
#        ${feat_tr_dir}/storage
#    fi
    if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d ${feat_dt_dir}/storage ]; then
    utils/create_split_dir.pl \
        /export/b{14,15,16,17}/${USER}/espnet-data/egs/librispeech/asr-unpaired/dump/${train_dev}/delta${do_delta}/storage \
        ${feat_dt_dir}/storage
    fi
...
#        data/taco_${train_set}/feats.scp data/taco_${train_set}/cmvn.ark exp/dump_feats/train ${taco_feat_tr_dir}
#    dump.sh --cmd "$train_cmd" --nj 32 --do_delta false \
#        data/taco_${train_dev}/feats.scp data/taco_${train_set}/cmvn.ark exp/dump_feats/dev ${taco_feat_dt_dir}
#    for rtask in ${recog_set}; do
