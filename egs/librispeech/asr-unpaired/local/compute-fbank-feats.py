...
def main():
    ...
    with open(args.scp, 'r') as f:
        scp = [
            (x.rstrip().split()[0], " ".join(x.rstrip().split()[1:]))
    ...
    with kaldi_io_py.open_or_fd(arkscp, 'wb') as f:
        for idx, (utt_id, path) in enumerate(scp, 1):
            ...
            # Detect pipe. If is a decompress flac pipe read pipe, otherwise
            # die
            if path[-1] == '|':
                if path.split()[-2][-5:] == '.flac':
                    x, fs = librosa.core.load(path.split()[-2], sr=None)
                else:
                    raise Exception(
                        "Only decompressing to flac flag supported"
                        "Found this pipe in %s \n\n%s\n" % (args.scp, path)
                    )
            else:
                x, fs = librosa.core.load(path, sr=None)
            assert fs == args.fs, "Wrong sample frequency"
